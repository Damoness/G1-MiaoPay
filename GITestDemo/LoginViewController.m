...
- (void)textFieldChange:(NSNotification *)note{
    ...
    if (field.tag == 10) {
        ...
        NSMutableDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:KdengLuInfo];
        NSString *str = [dict objectForKey:field.text];
...
//        [self performSegueWithIdentifier:@"loginToHome" sender:self];
    ...
    [manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [self hideProgressAfterDelaysInSeconds:3 withCompletion:^{
            [_loding dismissViewControllerAnimated:NO completion:nil];
            ...
            if(code == 0){
                ...
                NSString *lastLoginPhoneNo = [[NSUserDefaults standardUserDefaults] stringForKey:kLastLoginPhoneNo];
                isOldUser  = [lastLoginPhoneNo isEqualToString:self.phoneNo.text];
                
                [[NSUserDefaults standardUserDefaults] setObject:self.phoneNo.text forKey:kLastLoginPhoneNo];
                ...
                if (b) {
//                    NSMutableDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:KdengLuInfo];
                    NSMutableDictionary*dict =[NSMutableDictionary dictionaryWithDictionary:[[NSUserDefaults standardUserDefaults] objectForKey:KdengLuInfo]];
                    NSLog(@"####%@",dict);
                    if (dict == nil) {
                        dict = [NSMutableDictionary dictionary];
                        
                    }
                    if (![self IsHasKeyInDict:dict string:self.phoneNo.text]) {
                        NSLog(@"%d",[self IsHasKeyInDict:dict string:self.phoneNo.text]);
                        NSLog(@"%@%@",self.password.text,self.phoneNo.text);
                        
                        
                        [dict setValue:self.password.text forKey:self.phoneNo.text];
//                       

                    }

                    
                    [[NSUserDefaults standardUserDefaults] setObject:dict forKey:KdengLuInfo];
                    NSLog(@"~~~%@",dict);
                }
...
- (BOOL)IsHasKeyInDict:(NSMutableDictionary *)dict string:(NSString *)str{
    
    for (NSString *ss in [dict allKeys]) {
        if ([str isEqualToString:ss]) {
            return YES;
        }
    }
    return NO;
